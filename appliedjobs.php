...
<head>
  ...
  <link href="lib/bootstrap/css/bootstrap.min.css" rel="stylesheet">
    ...
    <style>
        ...
        body{
            background:url("https://assets.hongkiat.com/uploads/40-cool-abstract-and-background-photoshop-tutorials/yellow-green-abstract.jpg"); /*url("img/intro-carousel/34.jpg");*/
...
<body id="back">
   ...
   <table style="width:100%;"> 
		...
		function apply(jid)
		{
			...
			// for searching the specific  internship
			...
			if(isset($_POST['sub']))
			   ...
			   // $query="select application.*,internship.* from application join internship on i_id = jid where application.user_s_name like '%".$username."%'";	
				$result=mysqli_query($link,"select application.*,internship.*,companies.* from application join internship on i_id=jid  join companies on cid=c_id where name like '%".$k."%' or c_email like '%".$k."%'  or jdesc like '%".$k."%'  or loc like '%".$k."%' or address like '%".$k."%' or sal like '%".$k."%'") or die("failed to query database".mysqli_error($link));
                if ($result->num_rows > 0)                                                                                                                                                                                        //or uname like '%".$username."%'                               
				{
                    while($row = $result->fetch_assoc()) 
					{
						$jid=$row['jid'];
                         ...
                         echo "<tr><td><form action='' method='post'>".$row['jid']."<td>".$row['name']."<td>".$row['c_email']."<td>".$row['jdesc']."<td>".$row['loc']."<td>".$row['address']."<td>".$row['sal']."</td><td>".
			...
			else if(!isset($_POST['sub']))
			{
				$conn=mysqli_connect("localhost","root","","intern") or die("connection failed".mysqli_connect_error());
			     $query="select application.*,internship.*,companies.* from application join internship  on i_id = jid join companies on cid=c_id where application.user_s_name like '%".$username."%'";//query
                //$query = "select * from internship where uname like '%".$username."%'";
				//......$x= $_COOKIE['x'];
