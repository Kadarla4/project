<?php
	                             // the session is used to store user specific data and create variables[global variables] ...
                                   //they can be used across multiple pages...
                                   //once session started or resumed you can create var and use them
	session_start();
if(isset($_SESSION['empname']))   // if the employee is already logged in and wants redirect to login page it  is redirected to emphome.php
{                                  //if in any page like jobspost.php wants to redirect to emplogin.php , as you are already logged in you are 
	header("location:empwelcome.php"); // redirected to empwelcome.php  ....... as empname as it .......it has been set at a time of emp login...
}                                    // at time of logging out  the session is unset and destroyed ...
                                      
?>                              <!--   here header function used to redirecting to "location:url" after satisying the condition  -->
                                  <!-- but here we are not redirecting to login page in any case so it is that neccessary-->
<!doctype html>
<html>
<head>
<meta charset="utf-8">
...
if(isset($_POST['submit']))
{
 ...
 $pd=$su.sha1($_POST['pass']).$scu;//masking the password in a hidden  format
...
$result=mysqli_query($link,"select * from employee where eid='$username' and pass='$pd'") or die("failed to query database".mysqli_error($link));
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		else
		{
			 ...
			 echo "<script>location.href='jobpost.php'</script>";
		}                //no need of empwelcome.php
	}
	else
	{
		echo "<script>alert('Invalid ID or Password. You are in Employee login page if you are not employee then go to other Login page.')</script>";
	}
